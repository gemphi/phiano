...
pub fn sort_by_phase(facet: &Facet, words: &[String]) -> Vec<String> {
...
pub fn sort_test(facet: &Facet) -> bool {
    let words: Vec<String> = facet.lexicon.keys().take(32).cloned().collect();
    if words.len() < 3 {
        return false;
    }

    let sorted = sort_by_phase(facet, &words);
...
pub fn sort_by_amplitude(facet: &Facet, words: &[String]) -> Vec<String> {
    let mut indexed: Vec<(String, f64)> = words
        .iter()
...
pub fn sort_by_sector(facet: &Facet, words: &[String], n_sectors: u16) -> Vec<String> {
