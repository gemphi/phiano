...
pub fn program_centric_analogy(facet: &Facet, source: &str, target: &str) -> ProgramAnalogyResult {
    let source_rels = extract_structure(facet, source);
    let target_rels = extract_structure(facet, target);
...
pub fn shares_structure(facet: &Facet, s1: &str, s2: &str) -> bool {
    let r1 = extract_structure(facet, s1);
    let r2 = extract_structure(facet, s2);
