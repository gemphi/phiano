...
impl Help {
    ...
    pub fn apply(&self, _ctx: &mut Context) -> bool {
        ...
        println!("  instruct \"instruction\"     - Execute prompt via instruction engine (verbose)");
        println!("  chat \"question\"            - Clean conversational response (no diagnostics)");
        println!("  chat                       - Interactive multi-turn chat mode");
        println!("  chat --verbose \"question\"  - Chat with cognitive metadata");
        ...
        println!("  cognitive \"sentence\"       - Full 16-agent cognitive analysis (Searle pipeline)");
