...
pub fn phase_heuristic(facet: &Facet, examples: &[(String, String)]) -> Vec<f64> {
    ...
    for (input, output) in examples {
        ...
        let input_phase = mean_phase(facet, &input_tokens);
        let output_phase = mean_phase(facet, &output_tokens);
...
pub fn suggest_structure(facet: &Facet, examples: &[(String, String)]) -> Vec<&'static str> {
