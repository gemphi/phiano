...
use crate::data::preprocess::sentence_to_wave;
...
pub fn is_ood(facet: &Facet, prompt: &str, threshold: f64) -> bool {
    ood_score(facet, prompt) > threshold
}
...
pub fn ood_score(facet: &Facet, prompt: &str) -> f64 {
    if facet.lexicon.is_empty() {
        return 1.0;
    }

    let (prompt_x, prompt_y) = sentence_to_wave(facet, prompt);
...
mod tests {
    ...
    fn test_ood_score() {
        ...
        let in_dist = ood_score(&facet, "rust code memory");
        let out_dist = ood_score(&facet, "quantum entanglement relativity");
