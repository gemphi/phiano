...
impl Regularization {
    ...
    pub fn amplitude_decay(facet: &mut Facet, decay_rate: f64) {
    ...
    pub fn phase_jitter(facet: &mut Facet, jitter: f64) {
    ...
    pub fn band_regularization(facet: &mut Facet, max_band: u32) {
        for phasor in facet.lexicon.values_mut() {
...
mod tests {
    ...
    fn test_amplitude_decay() {
        ...
        Regularization::amplitude_decay(&mut facet, 0.1);
    ...
    fn test_band_regularization() {
        ...
        Regularization::band_regularization(&mut facet, 10);
        for p in facet.lexicon.values() {
            assert!(p.band_n <= 10);
        }
    }
