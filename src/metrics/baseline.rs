...
pub fn random_baseline(facet: &Facet, prompt: &str) -> f64 {
...
pub fn frequency_baseline(facet: &Facet, prompt: &str) -> f64 {
    let evaluator = Evaluator::new();
...
pub fn phase_baseline(facet: &Facet, prompt: &str) -> f64 {
    let evaluator = Evaluator::new();
...
pub fn all_baselines(facet: &Facet, prompt: &str) -> (f64, f64, f64) {
    (
        random_baseline(facet, prompt),
        frequency_baseline(facet, prompt),
        phase_baseline(facet, prompt),
    )
}
