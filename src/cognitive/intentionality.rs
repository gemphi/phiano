...
impl IntentionalityAgent {
    pub fn process(facet: &Facet, prompt: &str) -> AgentContribution {
        ...
        let (phase, about, confidence) = match known.is_empty() {
            true => (0.0, "unknown topic".to_string(), 0.0),
            false => {
...
impl AboutnessAgent {
    pub fn process(facet: &Facet, prompt: &str) -> AgentContribution {
        ...
        let output = match grounded.is_empty() {
            true => "No referents found - all words unknown".to_string(),
            false => grounded.join("; "),
        };
        ...
        AgentContribution {
            ...
            confidence: match grounded.is_empty() {
                true => 0.0,
                false => 1.0,
            },
            output,
